...
void LongestIOFirst(Queue *ready, ProcessData **running, int current_time, int preemptive, Queue *state) { // nonpreemptive
    // ready queue가 비어 있는 경우
    if (ready->front == NULL) return;
    ...
    if (preemptive && *running != NULL) {
        ...
        int io_front = -2;
        for (int i = 0; i < MAX_IO_NUM; i++) { // 가장 가까운 시간의 io burst 탐색
            if ((*running)->io_timing[i] != -1 && ((*running)->cpu_burst - (*running)->remaining_time) < (*running)->io_timing[i]) {
                io_running = (*running)->io_burst[i];
                break;
            }
        }
        for (int i = 0; i < MAX_IO_NUM; i++) { // 가장 가까운 시간의 io burst 탐색
            if (ready->front->p_process->io_timing[i] != -1 && (ready->front->p_process->cpu_burst - ready->front->p_process->remaining_time) < ready->front->p_process->io_timing[i]) {
                io_front = (*running)->io_burst[i];
                break;
            }
        }
        if (ready->front->       <= io_running) return; // running이 더 길면 x
...
void Schedule(int alg_id, ProcessData p[], int pNum, int tq) {
    ...
    for (int current_time = 0; !IsAllTerminated(p, pNum); current_time++) {
        ...
        prev_running = (running != NULL); // running이 NULL이면 0, 그렇지 않으면 1
    }
